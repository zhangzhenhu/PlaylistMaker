...
void QMediaPlaylist::addMedia(const QUrl &content) {
    ...
    if (d->playlist.contains(content)) {
        return;
    }
...
QList<QUrl> QMediaPlaylist::difference(const QList<QUrl> &items) {
    if (items.empty()) {
        return items;
    }
    Q_D(QMediaPlaylist);
...
bool QMediaPlaylist::removeMedia(const QUrl &content) {
    Q_D(QMediaPlaylist);
    int pos = d->playlist.indexOf(content);
    if (pos == -1) {
        return false;
    };
    return this->removeMedia(pos,pos);
//    emit mediaAboutToBeRemoved(pos, pos);
//    d->playlist.remove(pos, pos);
//    d->check_state.remove(content);
...
//排列判断
static int compare_path(const QUrl &url1, const QUrl &url2) {
    return url1.path() < url2.path();
}

static int compare_name(const QUrl &url1, const QUrl &url2) {
    return url1.fileName() < url2.fileName();
}

static int compare_path_descending(const QUrl &url1, const QUrl &url2) {
    return url1.path() > url2.path();
}

static int compare_name_descending(const QUrl &url1, const QUrl &url2) {
    return url1.fileName() > url2.fileName();
}

void QMediaPlaylist::sort(int column, Qt::SortOrder order) {
    Q_D(QMediaPlaylist);
    if (order == Qt::AscendingOrder) {
        if (column == 0) {
            std::sort(d->playlist.begin(), d->playlist.end(), compare_name);

        } else {
            std::sort(d->playlist.begin(), d->playlist.end(), compare_path);

        }
    } else {

        if (column == 0) {
            std::sort(d->playlist.begin(), d->playlist.end(), compare_name_descending);

        } else {
            std::sort(d->playlist.begin(), d->playlist.end(), compare_path_descending);

        }
    }
    emit mediaChanged(0, this->mediaCount());
}

//void QMediaPlaylist::setCheckStat(int index, Qt::CheckState state) {
//    Q_D(QMediaPlaylist);
//    auto url = this->media(index);
//    if (url.isEmpty()) {
//        return;
//    }
//    d->check_state[url] = state;
//}
//
//Qt::CheckState QMediaPlaylist::checkStat(int index) {
//    Q_D(QMediaPlaylist);
//    auto url = this->media(index);
//
//    if (url.isEmpty() || !d->check_state.contains(url)) {
//        return Qt::CheckState::Unchecked;
//    }
//    return d->check_state[url];
//}
//
//void QMediaPlaylist::clearCheckedMedias() {
//    Q_D(QMediaPlaylist);
//    auto iter = d->check_state.begin();
//    QList<QUrl> pool;
//    while (iter != d->check_state.end()) {

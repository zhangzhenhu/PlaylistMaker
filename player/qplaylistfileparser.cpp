...
namespace {
    ...
    public:
        explicit ParserBase(QPlaylistFileParser *parent)
                : m_parent(parent), m_aborted(false) {
            Q_ASSERT(m_parent);
        }
    ...
    public:
        explicit M3UParser(QPlaylistFileParser *q)
                : ParserBase(q), m_extendedFormat(false) {
        }
...
public:
    QPlaylistFileParserPrivate(QPlaylistFileParser *q)
            : q_ptr(q), m_stream(nullptr), m_type(QPlaylistFileParser::UNKNOWN), m_scanIndex(0), m_lineIndex(-1),
              m_utf8(false), m_aborted(false) {
    }
...
bool QPlaylistFileParserPrivate::processLine(int startIndex, int length) {
    ...
    if (!m_currentParser) {
        ...
        m_type = QPlaylistFileParser::findPlaylistType(suffix, !mimeType.isEmpty() ? mimeType : m_mimeType,
                                                       m_buffer.constData(), quint32(m_buffer.size()));
        switch (m_type) {
...
QPlaylistFileParser::FileType QPlaylistFileParser::findByDataHeader(const char *data, quint32 size) {
    if (!data || size == 0)
        return QPlaylistFileParser::UNKNOWN;
    if (size >= 8 && strncmp(data, "#EXTM3U8", 8) == 0)
        return QPlaylistFileParser::M3U8;
